...
{
    if (!_colorBackView) {
        _colorBackView = [[MYColorBackView alloc]initWithFrame:CGRectMake(0, STATUS_BAR_HEIGHT + 44, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - STATUS_BAR_HEIGHT - 44)];
        if (self.isNeedClearBack) {
            ...
            if (_showFrame.origin.y > (STATUS_BAR_HEIGHT + 44)) {
