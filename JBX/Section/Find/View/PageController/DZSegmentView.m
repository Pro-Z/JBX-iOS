...
//
//  Created by 证 on 2017/11/23.
//  Copyright © 2017年 证. All rights reserved.
//

#import "DZSegmentView.h"

typedef void(^IndexBlock)(NSUInteger ,UIButton *);
int const SegmentHeight = 48;//可根据项目需求设置高度

@interface DZSegmentView ()
...
- (void)segmentBasicSetting {
    ...
    _segmentNormalColor = RGBA(101, 101, 101, 1);
    _font = PINGFANG_FONT_SIZE(15);
...
- (void)adjustOffsetXToFixIndicatePosition:(CGFloat)offsetX {
    ...
    CGFloat originMovedX = _style == SegmentStyleDefault? (CGRectGetMinX(_selectedButton.frame) + _buttonSpace) : CGRectGetMinX(_selectedButton.frame);
    CGFloat targetMovedWidth = [self widthAtIndex:currentIndex];//需要移动的距离
    
    CGFloat targetButtonWidth = _style == SegmentStyleDefault? ([self widthAtIndex:buttonIndex] - 2 * _buttonSpace) : [self widthAtIndex:buttonIndex]; // 这个会影响width
    CGFloat originButtonWidth = _style == SegmentStyleDefault? ([self widthAtIndex:[self index]] - 2 * _buttonSpace) : [self widthAtIndex:[self index]];
...
- (void)setStyle:(SegmentStyle)style {
    _style = style;
    
    if (style==SegmentStyleFlush) {
